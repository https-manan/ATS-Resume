...
from frontend.components._helper import get_severity_style
...
def _render_issue(issue: Dict[str, Any]) -> None:
    label, text_color, bg_color = get_severity_style(issue.get("severity_level"))
    ...
    st.markdown(
        f"""
        <div style="border-left:3px solid {text_color}; background-color:{bg_color};
