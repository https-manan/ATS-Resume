...
if (
    ...
    from frontend.services import supabaseClient
    result = supabaseClient.exchange_code_for_session(st.query_params["code"])
...
with st.sidebar:
    ...
    if st.session_state.access_token:
        ...
        with tab_in:
            ...
            if submitted:
                result = supabaseClient.sign_in_with_password(email, password)
                if "error" in result:
        ...
        with tab_up:
            ...
            if submitted_up:
                result = supabaseClient.sign_up_with_password(email_up, password_up)
                if "error" in result:
                    st.session_state.auth_error = result["error"]
        ...
        oauth = supabaseClient.google_oauth_url()
        if "error" in oauth:
