...
LOG_DIR = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'logs')  #basically ek directory mai saare logs daal dega and naahi exist krti to bna kr logs daal dega uss dir mai 
...
class FileParsingError(ATSBaseError):#File parsing error is for like ki can file be able to parse properly 
    pass
...
#These are teh basic and by default values we gonna call later in the main file 
