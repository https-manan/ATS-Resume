#This gonna recommend in orders like these are the urgent and these are and all like whats we gotta work on 1sst and 2nd .....
...
#basically this is for user ko kiss level ka error show krana hai
class Priority(Enum):
    CRITICAL='critical',
    HIGH='high',
    MEDIUM='medium',
...
@dataclass  #The @dataclass decorator in Python is used to automatically generate standard special methods—like __init__, __repr__, and __eq__
class Recommendation: #This recommendation class is for like what we gonna rec to the user 
    title:        str
    description:  str
    priority:     Priority  #This is for jo aabhi feedback de rhe hai uski proiroty kitni hai 
    impact_score: float
    category:     str
...
#generator01
#Here we gonna generate the skills that are lagging means that that skills which are not presented in the skills section but they are present in the projects that we gonnaa rec for skills 
def generate_skill_recommendations(skill_validation_results: Dict) -> List[Recommendation]:
    ...
    if not unvalidated:   #Means agr sb sahi hai like sari skills are validated in projects to good we gonna return empty list of recomendations
        return recommendations

    if validation_pct < 0.4:   # means less than 40%
        priority, impact = Priority.CRITICAL, 8.0 # And agr less than 40% hai to we gonna say critical menas 40% ka diff hai in skills and projects
    elif validation_pct < 0.6:
        priority, impact = Priority.HIGH, 6.0    #And same  
    elif validation_pct < 0.8:
        priority, impact = Priority.MEDIUM, 4.0
    else:
        priority, impact = Priority.LOW, 2.0

    #basically jo unvalidated skills hai usme se top 5 he user ko show krenge if size of unvalid is >5
    action_items = [f"Add a project or experience demonstrating '{skill}', or remove it from skills"
                    for skill in unvalidated[:5]]

    if len(unvalidated) > 5:
        action_items.append(f'... and {len(unvalidated) - 5} more unvalidated skill(s)')

    #Agr 5 se km hai to simply add all and return 
...
#generator02 
#  grammtical suggestions
def generate_grammar_recommendations(grammar_results: Dict) -> List[Recommendation]:
    recommendations = []  #Yha pr resume ke gramatical errors add krenge rec me
    ...
    if total == 0:
        return recommendations  #basically koi error nahi hai to return [] empty list 


    #This is rec for all critical errors
    if critical_errors:
        items = []
        for error in critical_errors[:5]:   #Basically top 5 from critical rec and then all
    ...
    #Same yha pr moderate errors ko rec krenge 3 ko 1st 
    if moderate_errors:
        items = []
    ...
    #Same yha pr minor errors ko rec krenge 3 ko 1st 
...
#generator03
#  location recommendations basically yha hum agr location hai to use rec krenge hta le and all 
def generate_location_recommendations(location_results: Dict) -> List[Recommendation]:
    ...
    if privacy_risk == 'none' or not detected_locations:  #Agr loc nahi hai to good return [] empty list 
        return recommendations
    ...
    #Yhe pr bhi same humne 3 priority mai baata hai ki :-critical hai,high hai and medium hai 
...
#this rec is for keyword rec ki like agr kuch missing hai ya gap hai and all.
...
def generate_formatting_recommendations(score_results: Dict,sections: Dict[str, str],)->List[Recommendation]:
    ...
    section_recommendations = {   #Basically checking ki ye saare sections exist krne chiya resumee m
    ...
    #Core mai saare main main included like exp,edu and skills and all and agr vo he koi missing hai then this part .
    if core_missing:
        recommendations.append(Recommendation(
...
#This gonna be the index number of the priority we gonna send/recomend the issues in this order 
...
#orchestrator of this whole file means saare issues ko call krega and all 
...
#Basicaally this is for formating like criticaal hai to red and sbse upper and all 
...
#This is for all at once groups me issues nahi bhajenge saare ek saath bhj denge  in a json file 
