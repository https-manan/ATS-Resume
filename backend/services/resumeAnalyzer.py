...
from backend.services.JdMatcher import compare_resume_with_jd
from backend.services.feedback import analyze_issues, generate_issues_summary
...
def analyze_full_resume(resume_text: str, nlp: spacy.Language, embedder: Optional[RemoteEmbedder], job_description: Optional[str] = None,) -> Dict:
    ...
    #SentenceTransformer instance (loaded once at startup in main.py and passed in as `embedder`)
    #with the model's file path string from config. Every downstream `embedder.encode(...)` call
    #(in JdMatcher.py, atsScorer.py) would then fail with AttributeError since strings have no .encode().
