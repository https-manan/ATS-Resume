#Here we gonna analyze whats specifically wring in the resume
...
#
def analyze_issues(resume_text:str,parsed_resume:Dict,skills:List[str],projects:List[Dict],action_verbs:List[str],skill_validation:Dict,scores:Dict,contact_info:Optional[Dict]=None)->List[IssueDetail]:
    detected:List[IssueDetail]=[]#This is for storing ki saari entries actually hai ya nahi and all
    ...
    #Building a combination exp based text for a regex-based checks that still needs text
    exp_text='\n'.join(e.get('description','') for e in exp_entries).strip()

    resume_lower=resume_text.lower()
    has_perojects_signal=any(kw in resume_lower for kw in [
        'project','github.com','deployed','build a','developed a','created a',
        'implemented a','live demo','teck stack'])

    
    #Basically yha pe this only gonna fire the feedback and all if not projects ki resume
    if not proj_entries and len(projects)==0 and not has_perojects_signal:
        detected.append(IssueDetail(
            ...
            action_items=[
                ...
                "For each project: write the title, tech stack used, what you built",
                "Example: 'E-Commerce Platform – React, Node.js, MongoDB'",
                "Link to GitHub or a live demo if available (e.g., github.com/yourname)"
            ],
            example_improvement=(
                "Add:\n"
                "PROJECTS\n"
                "E-commerce platform - Built a full-stack shopping site using "
                "React, Node.js, MongoDB. Implemented payment processing with "
                "Razorpay, handling 500+ transactions.\n"
                "ML sentiment analysis using a fine-tuned BERT model."
            )
        ))

    #2. missing experience section
    has_experience_signal = any(kw in resume_lower for kw in [  #Here we have used these all keywords for detectinng ki user ne kahi kaam kiya hai ya nahi
    ...
    #Basically yha pe bhi this only gonna fire the feedback and all if not exp ki entries
    if not exp_entries and not has_experience_signal:
        detected.append(IssueDetail(
            ...
            where_it_appears="Resume structure – no 'Experience' or 'Work History'",
            how_to_fix=(
            ...
            action_items=[
                ...
                "If no formal job: include internships, freelance work, open-source contributions",
                "Start every bullet with a past-tense action verb (Developed, Built, Improved, etc.)"
            ],
            example_improvement=(
                "Add:\n"
                "EXPERIENCE\n"
    ...
    #Simiarly we gonnna look for education mentioned or not
    ...
    #If edu is not mentioned then we gonna trigger this feedback
    ...
    # Here we gonna check for missing Skills Section 
    ...
    # here we gonna search the proof of work for the skills teh user adds in his resume
    ...
    # Basically numbers bhi hai ya nahi like 20% and all...
    number_pattern = r'\d+[%+]?|\$\d+'
    has_metrics = bool(re.findall(number_pattern, exp_text)) if exp_text else False

    if exp_text and not has_metrics:
        detected.append(IssueDetail(
    ...
    #checking for Missing Contact Information 
    if contact_info:
        missing_contacts = []
    ...
    # Missing Summary/Objective of the skills or experience or anything
    if not summary:
        detected.append(IssueDetail(
