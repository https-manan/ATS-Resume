...
from backend.models.schema import AnalysisResponse, ComponentScores, JDComparison, SkillValidationDetails
from backend.utils.file_utils import (get_default_grammar_results,get_default_location_results,get_default_skill_validation_results,)#Baiscalyy these are for passing the default values to the user

logger = logging.getLogger('ats_resume_scorer')

router = APIRouter(prefix='/api/v1', tags=['Analysis'])

#Basically agr parsed text mai koi bhi emoji aa rha hai to 1st remove that.
...
#This is for analyze resume route
@router.post('/analyze-resume', response_model=AnalysisResponse)
async def analyze_resume(request: Request,resume: UploadFile = File(..., description='Resume file — PDF or DOCX, max 5 MB'),
                         job_description: str = Form('', description='Job description text (optional)'),user_id: str = Depends(get_current_user),):
    warnings: List[str] = []
    ...
    try:
        ...
        from backend.services.resumeParser import (
            FileParsingError,
            FileValidationError,
            parse_resume_file,
        )
    ...
    try:
        from backend.services.resumeAnalyzer import analyze_full_resume
    ...
    from backend.models.schema import ComponentScores
...
#This is to check the health of the app 
@router.get('/health')
...
async def get_history(user_id: str = Depends(get_current_user)):
    ...
    from backend.database.db import get_user_history
    try:
...
async def delete_history_entry(
    ...
    from backend.database.db import delete_analysis
    try:
...
async def generate_pdf(
    ...
    from backend.services.reportGenerator import generate_html_reports
    from backend.services.pdfExporter import generate_combined_pdf
