...
async def save_analysis(user_id: str, filename: str, analysis_result: Dict) -> Optional[str]:
    ...
    doc = {
        ...
        "filename": filename,#Kunsi file thi 
        ...
        "analysis_result": serializable_result, #And then the result of that user's file
    }
...
async def delete_analysis(analysis_id: str, user_id: str) -> bool:
    ...
    try:
        async with httpx.AsyncClient() as client:
            response = await client.delete(    #Sending del req with id and user_id
                url, 
                headers=headers, 
                params={
                    "id": f"eq.{analysis_id}",
